...
class Generator(nn.Module):
    def __init__(self, dataset='mnist', model='cnn', embedding=False, latent_layer_idx=-1,
                 problem_type='classification', n_class=None):
    ...
    def init_loss_fn(self):
        ...
        self.l1_loss = nn.L1Loss(reduce=False)  # same as above
    ...
    def build_network(self):
        ...
        if self.problem_type == 'regression':
            self.regression_input_embedding = nn.Linear(self.n_class, self.noise_dim)
    ...
    def forward(self, labels, latent_layer_idx=-1, verbose=True):
        ...
        eps = torch.rand((batch_size, self.noise_dim), device=labels.device)  # sampling from Gaussian
        if verbose:
            result['eps'] = eps
        if self.problem_type == 'classification':
            ...
            y_input = self.regression_input_embedding(labels)
