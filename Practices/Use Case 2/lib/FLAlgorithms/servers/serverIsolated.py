...
class Isolated(Server):
    def __init__(self, args, model, seed):
        ...
        random_user_id = np.random.choice(total_users)
        #random_user_id = 1
        random_user_train_data = None
        random_user_test_data = None
        for i in range(total_users):
            id, train_data, test_data = read_user_data(i, data, dataset=args.dataset, model=args.model)
            if random_user_id == i:
                random_user_train_data = train_data
                random_user_test_data = test_data
            #aggregated_test_data.extend(test_data)
        user = UserCentralized(args, 0, model, random_user_train_data, random_user_test_data, use_adam=True)
        self.users.append(user)
        self.total_train_samples += user.train_samples

        print("Train user ID:", random_user_id)
        print("Number of users / total users:", args.num_users, " / ", total_users)
        print("Finished creating Isolated server.")
