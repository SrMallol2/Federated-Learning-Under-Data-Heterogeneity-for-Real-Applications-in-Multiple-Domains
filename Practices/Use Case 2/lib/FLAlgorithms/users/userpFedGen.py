...
class UserpFedGen(User):
    def __init__(self,
        ...
        self.output_range = args.output_range
    ...
    def update_y_distribution(self, mean, std, n):
        if self.y_distribution['mean'] is None:
            self.y_distribution['mean'] = mean
            self.y_distribution['std'] = std
            self.y_distribution['n'] = n
            return
        old_mean = self.y_distribution['mean']
        old_std = self.y_distribution['std']
        old_n = self.y_distribution['n']
        aggregated_mean = (old_n * old_mean + n * mean) / (old_n + n)
        aggregated_std = math.sqrt(
            ((old_std ** 2 + old_mean ** 2) * old_n + (std ** 2 + mean ** 2) * n) / (old_n + n) - (
                    aggregated_mean ** 2))
        self.y_distribution['mean'] = aggregated_mean
        self.y_distribution['std'] = aggregated_std
        self.y_distribution['n'] = old_n + n

    def clean_up_y_distribution(self):
        self.y_distribution = {
            'mean': None,
            'std': None,
            'n': 0
    ...
    def train(self, glob_iter, personalized=False, early_stop=100, regularization=True, verbose=False):
        ...
        for epoch in range(self.local_epochs):
            ...
            for i in range(self.K):
                ...
                samples = self.get_next_train_batch(return_y_distribution=True)
                ...
                if self.problem_type == 'classification':
                    ...
                    self.update_y_distribution(samples['mean'], samples['std'], samples['n'])
                ...
                if regularization and epoch < early_stop:
                    ...
                    if self.problem_type == 'classification':
                        ...
                        #TODO: probably we can consider other types of loss functions too
                        teacher_loss = generative_alpha * torch.mean(
                            self.generative_model.l1_loss(user_output_logp, sampled_y)
                        )
