...
#################################
class Net(nn.Module):
    def __init__(self, dataset='mnist', model='cnn', problem_type='classification', output_range=None, output_dimension=None):
        super(Net, self).__init__()
        # define network layers
        print("Creating model for {}".format(dataset))
        self.dataset = dataset
        self.problem_type = problem_type
        configs, input_channel, self.output_dim, self.hidden_dim, self.latent_dim = CONFIGS_[f'{dataset}-{model}']
    ...
    def build_network(self, configs, input_channel, output_dim):
        ...
        for i, x in enumerate(configs):
            if x == 'F':
                ...
                layers += [pool_layer]
                layer_names += [layer_name]
            elif type(x) is str and x.startswith('LSTM'):
                hidden_size = int(x.split('-')[1])
                layer_name = 'lstm{}'.format(i)
                layer = nn.LSTM(input_size=input_channel, hidden_size=hidden_size,
    ...
    def forward(self, x, start_layer_idx=0, logit=False):
        ...
        for idx in range(start_layer_idx, len(self.layers)):
            ...
            if 'lstm' in layer_name:
                h_0 = Variable(torch.zeros(layer.num_layers, x.size(0), layer.hidden_size, device=x.device))
                c_0 = Variable(torch.zeros(layer.num_layers, x.size(0), layer.hidden_size, device=x.device))
                layer.flatten_parameters()
                _, (z, _) = layer(z, (h_0, c_0))
                z = z.view(-1, layer.hidden_size)
                continue
            z = layer(z)

        if self.output_dim > 1 and self.problem_type == 'classification':
            results['output'] = F.log_softmax(z, dim=1)
        elif self.problem_type == 'regression':
            z = F.sigmoid(z)
            if self.output_range is not None:
                z = z * (self.output_range[1] - self.output_range[0]) + self.output_range[0]
            results['output'] = z
        if logit:
            results['logit'] = z
        return results
