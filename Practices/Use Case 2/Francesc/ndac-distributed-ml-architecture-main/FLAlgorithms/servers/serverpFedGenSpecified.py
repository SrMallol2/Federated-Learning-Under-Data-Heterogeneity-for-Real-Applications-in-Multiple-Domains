...
from utils.utils import append_statistic_to_file

MIN_SAMPLES_PER_LABEL = 1


class FedGenSpecified(Server):
    def __init__(self, args, model, seed):
        ...
        self.generative_model = create_generative_model(args.dataset, args.device, args.algorithm, self.model_name,
                                                        args.embedding,
                                                        args.problem_type,
                                                        args.steps)
        if not args.train:
        ...
        for i in range(total_users):
            id, train_data, test_data, label_info = read_user_data(i, data, dataset=args.dataset, count_labels=True,
                                                                   model=args.model)
            if i == 0:
                self.test_data = train_data[int(0.01 * len(train_data)):] + test_data
                self.total_test_samples = len(self.test_data)
                train_data = train_data[:int(0.01 * len(train_data))]
    ...
    def train(self, args):
        ...
        for glob_iter in range(self.num_glob_iters):
            ...
            if self.problem_type == 'classification':
                ...
                self.train_generator_regression(
                    self.batch_size,
                    epoches=self.ensemble_epochs // self.n_teacher_iters,
                    latent_layer_idx=self.latent_layer_idx,
                    verbose=True,
        ...
        def update_generator_(n_iters, student_model, TEACHER_LOSS, STUDENT_LOSS, DIVERSITY_LOSS):
            ...
            for i in range(n_iters):
                ...
                for user_idx, user in enumerate(self.selected_users):
                    ...
                    teacher_loss_ = torch.mean(self.generative_model.crossentropy_loss(user_output_logp_, y_input) * \
    ...
    def train_generator_regression(self, batch_size, iteration, epoches=1, latent_layer_idx=-1, verbose=False):
        """
        Learn a generator that find a consensus latent representation z, given ground truth.
        :param batch_size:
        :param iteration:
        :param epoches:
        :param latent_layer_idx: if set to -1 (-2), get latent representation of the last (or 2nd to last) layer.
        ...
        """
        # self.generative_regularizer.train()
        self.gmm_of_users = self.get_gmm_of_users()
        if verbose:
            append_statistic_to_file('./statistics/gmm_of_users.pkl', self.gmm_of_users, iteration)
        TEACHER_LOSS, STUDENT_LOSS, DIVERSITY_LOSS, STUDENT_LOSS2 = 0, 0, 0, 0
        ...
        def update_generator_(n_iters, student_model, TEACHER_LOSS, STUDENT_LOSS, DIVERSITY_LOSS):
            ...
            for i in range(n_iters):
                self.generative_optimizer.zero_grad()
                output_dimension = student_model.layers[-1].out_features
                y, _ = self.gmm_of_users.sample(batch_size * output_dimension)
                y_input = torch.FloatTensor(y).view((batch_size, output_dimension))
                ...
                for user_idx, user in enumerate(self.selected_users):
                    user.model.eval()
                    user_result_given_gen = user.model(gen_output, start_layer_idx=latent_layer_idx, logit=True)
                    user_output_logp_ = user_result_given_gen['logit']
    ...
    def get_gmm_of_users(self):
        normal_distributions = []
        for user in self.selected_users:
            normal_distributions.append(
                normal(user.y_distribution['mean'], user.y_distribution['std'], user.y_distribution['n']))
        X = concatenate(normal_distributions).reshape(-1, 1)
        gm = GaussianMixture(n_components=len(self.selected_users), random_state=0).fit(X)
        return gm
