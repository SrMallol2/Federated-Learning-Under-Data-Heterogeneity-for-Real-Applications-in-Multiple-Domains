...
METRICS = ['glob_test_metric', 'per_acc', 'glob_loss', 'per_loss', 'user_train_time', 'server_agg_time']
...
def get_data_dir(dataset):
    if 'EMnist' in dataset:
        ...
        types = 'letters'
        path_prefix = os.path.join(f'u20-{types}-alpha{alpha}-ratio{ratio}')
        train_data_dir = os.path.join(path_prefix, 'train')
        test_data_dir = os.path.join(path_prefix, 'test')
        proxy_data_dir = 'proxy_data/emnist-n10/'

    elif 'Mnist' in dataset:
        dataset_ = dataset.replace('alpha', '').replace('ratio', '').split('-')
        alpha, ratio = dataset_[1], dataset_[2]
        # path_prefix=os.path.join('data', 'Mnist', 'u20alpha{}min10ratio{}'.format(alpha, ratio))
        path_prefix = os.path.join('u20c10-alpha{}-ratio{}'.format(alpha, ratio))
        train_data_dir = os.path.join(path_prefix, 'train')
        test_data_dir = os.path.join(path_prefix, 'test')
        proxy_data_dir = 'proxy_data/mnist-n10/'

    elif 'celeb' in dataset.lower():
        dataset_ = dataset.lower().replace('user', '').replace('agg', '').split('-')
        user, agg_user = dataset_[1], dataset_[2]
        path_prefix = os.path.join('user{}-agg{}'.format(user, agg_user))
        train_data_dir = os.path.join(path_prefix, 'train')
        test_data_dir = os.path.join(path_prefix, 'test')
        proxy_data_dir = os.path.join('/user500/', 'proxy')

    elif 'flagsregression' in dataset.lower():
        dataset_ = dataset.lower().replace('user', '').replace('alpha', '').replace('ratio', '').split('-')
        user, alpha, ratio = dataset_[1], dataset_[2], dataset_[3]
        path_prefix = os.path.join('u{}-alpha{}-ratio{}'.format(user, alpha, ratio))
        train_data_dir = os.path.join(path_prefix, 'train')
        test_data_dir = os.path.join(path_prefix, 'test')
        proxy_data_dir = 'proxy_data/flags_regression/'

    elif 'flags' in dataset.lower():
        dataset_ = dataset.lower().replace('user', '').replace('alpha', '').replace('ratio', '').split('-')
        user, alpha, ratio = dataset_[1], dataset_[2], dataset_[3]
        path_prefix = os.path.join('u{}-alpha{}-ratio{}'.format(user, alpha, ratio))
        train_data_dir = os.path.join(path_prefix, 'train')
        test_data_dir = os.path.join(path_prefix, 'test')
        proxy_data_dir = 'proxy_data/flags/'
    elif 'synreg' in dataset.lower():
        dataset_ = dataset.lower().replace('user', '').replace('alpha', '').replace('ratio', '').split('-')
        user, alpha, ratio = dataset_[1], dataset_[2], dataset_[3]
        path_prefix = os.path.join('u{}-alpha{}-ratio{}'.format(user, alpha, ratio))
        train_data_dir = os.path.join(path_prefix, 'train')
        test_data_dir = os.path.join(path_prefix, 'test')
        proxy_data_dir = 'proxy_data/synthetic_regression/'
...
def read_data(dataset, data_directory='data'):
    ...
    train_data_dir = os.path.join(data_directory, train_data_dir)
    test_data_dir = os.path.join(data_directory, test_data_dir)
    proxy_data_dir = os.path.join(data_directory, proxy_data_dir)
    clients = []
    groups = []
    train_data = {}
    test_data = {}
...
def read_proxy_data(proxy_data, dataset, batch_size, model=''):
    X, y = proxy_data['x'], proxy_data['y']
    X, y = convert_data(X, y, dataset=dataset, model=model)
...
def aggregate_data_(clients, dataset, dataset_name, batch_size, model=''):
    combined = []
    unique_labels = []
    for i in range(len(dataset)):
        id = clients[i]
        user_data = dataset[id]
        X, y = convert_data(user_data['x'], user_data['y'], dataset=dataset_name, model='')
...
def aggregate_user_test_data(data, dataset_name, batch_size, model=''):
    clients, loaded_data = data[0], data[3]
    data_loader, _, unique_labels = aggregate_data_(clients, loaded_data, dataset_name, batch_size, model=model)
    return data_loader, np.unique(unique_labels)


def aggregate_user_data(data, dataset_name, batch_size, model=''):
    # data contains: clients, groups, train_data, test_data, proxy_data
    clients, loaded_data = data[0], data[2]
    data_loader, data_iter, unique_labels = aggregate_data_(clients, loaded_data, dataset_name, batch_size, model=model)
    return data_loader, data_iter, np.unique(unique_labels)


def convert_data(X, y, dataset='', model=''):
    if not isinstance(X, torch.Tensor):
        if 'celeb' in dataset.lower():
            ...
            y = torch.Tensor(y).type(torch.int64)
    if ('cnn' in model.lower()) and ('flags' in dataset.lower() or 'synreg' in dataset.lower()):
        X = X[:, None, :, :]
    return X, y


def read_user_data(index, data, dataset='', count_labels=False, model=''):
    # data contains: clients, groups, train_data, test_data, proxy_data(optional)
    id = data[0][index]
    train_data = data[2][id]
    test_data = data[3][id]
    X_train, y_train = convert_data(train_data['x'], train_data['y'], dataset=dataset, model=model)
    train_data = [(x, y) for x, y in zip(X_train, y_train)]
    X_test, y_test = convert_data(test_data['x'], test_data['y'], dataset=dataset, model=model)
...
def get_dataset_name(dataset):
    ...
    elif 'flagsregression' in dataset:
        passed_dataset = 'flags-regression'
    elif 'flags' in dataset:
        passed_dataset = 'flags'
    elif 'synreg' in dataset:
        passed_dataset = 'synthetic-regression'
    else:
        raise ValueError('Unsupported dataset {}'.format(dataset))
    return passed_dataset


def create_generative_model(dataset, device, algorithm='', model='cnn', embedding=False, problem_type='classification',
                            n_class=None):
    ...
    if 'FedGen' in algorithm:
        ...
        if 'cnn' in algorithm:
            ...
            passed_dataset += '-cnn1'
    model = Generator(passed_dataset, model=model, embedding=embedding, latent_layer_idx=-1, problem_type=problem_type,
                      n_class=n_class)
    model = model.to(device)
    return model


def create_model(model, dataset, algorithm, problem_type, output_range, output_dimension):
    passed_dataset = get_dataset_name(dataset)
    model = Net(passed_dataset, model, problem_type, output_range, output_dimension), model
    return model
